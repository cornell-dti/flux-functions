...
async function computeNewWaitline(diningHall, day, numLines) {
  ...
  const lineLength =
    (await db.collection("waittimes").doc("lineLengths").get()).data()[
      diningHall
    ] || 0;
  ...
  const servingTimeForOnePerson =
    ((await db.collection("waittimes").doc("serviceRates").get()).data()
      .serviceRate_HC || 75 / 60) / numLines; // We got this data from cornell dining
  ...
  const weightedWait =
    feedbackData.weight * feedbackData.feedback +
...
async function waitTime(fnData) {
  ...
  const numLines = (
    await db.collection("waittimes").doc("numLines").get()
  ).data();
  return Promise.all(
    slugs.map((slug) => computeNewWaitline(slug, day, numLines[slug] || 1))
  );
}
